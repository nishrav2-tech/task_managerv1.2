...
alter table properties add column if not exists listing_url_2 text;
...
begin
  for t in
    select * from (values
      ...
      ('properties',    array['id','property_id','county','state','acres','buy_price','sell_price','closing_date','notes','funding_schedule','status','listing_url','listing_url_2','listing_stats','agent_log','ghl_url','land_portal_url','created_at']),
...
-- Row Level Security: see auth-lockdown.sql (kept identical below).
-- =========================================================
